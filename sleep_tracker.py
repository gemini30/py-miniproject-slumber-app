...
def start_tracking():
    ...
    while cap.isOpened():
        ...
        # Plot vector points at shoulder and nose
        image=cv2.circle(image,(int(nx),int(ny)),3,(255,0,0),3)
        image=cv2.circle(image,(int(srx),int(sry)),3,(0,255,0),3)
        image=cv2.circle(image,(int(slx),int(sly)),3,(0,255,0),3)
